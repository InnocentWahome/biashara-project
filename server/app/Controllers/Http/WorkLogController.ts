...
export default class WorkLogController {
  ...
  public async store({ request, response }: HttpContextContract) {
    try {
      const data = request.only(['user_id', 'description', 'hours', 'start', 'stop', 'date', 'day', 'approval'])
  ...
  public async update({ params, request, response }: HttpContextContract) {
    try {
      ...
      if (!worklog) {
        ...
        worklog.merge(request.only(['user_id', 'description', 'hours', 'start', 'stop', 'date', 'approval']))
        await worklog.save()
        return response.json({
          success: true,
